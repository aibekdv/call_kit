...
///
/// A drift does not crash. It produces a call the app can never end, because
/// it holds a different identifier than the system does.
///
/// This lives in the example's test target rather than beside the plugin
/// source because that target is the one Xcode actually builds and runs:
///
/// ```
/// cd packages/call_native_kit/example/ios
/// xcodebuild test -workspace Runner.xcworkspace -scheme Runner \
///   -destination 'platform=iOS Simulator,name=iPhone 16'
/// ```
class CallUuidTests: XCTestCase {
